...
This repository is a working implementation, not a claim that every deep rhetorical workflow or dedicated control surface is complete. The **Timeline / Relational Correction** milestone makes Workbench the default view: ordered section cards, first-class Storyboard notes, one editable prose preview, and section-local assistance. The current refinement tightens local boundary selections, makes Overview more compact, and clarifies prose-edit entry. Segue comparison reads adjacent sections without gaining permission to edit them. Word/Phrase Lens, the Personal Writing Library, scoped guides, and Structure remain available in the same application—not parallel editors or a new canonical store. See the [relational editing handoff](docs/RELATIONAL-EDITING.md), [refinement verification](docs/REFINEMENT-VERIFICATION.md), [workbench guide](docs/WORKBENCHES.md), [personal-library and Structure guide](docs/PERSONAL-LIBRARY.md), and [Known limitations](#known-limitations).
...
- **Workbench** is the primary default, including for older settings without a saved view. **Document** is the alternate view of the same editor. View, Comfortable/Overview density, preview visibility, and Inspector visibility persist as global layout preferences. Hide preview to give the cards more room; **Edit in preview** reveals the existing editor and places a collapsed caret at the start of that section. Select prose before replacing it.
- Comfortable cards show up to seven lines of prose. The selected card exposes its editable **Storyboard note**; other Comfortable notes preview four lines. Overview uses a one-line excerpt, a compact role and a selected-card note to make a longer arc scannable. Cards show semantic role and an explicit section-model override badge when set. **Add before / Add after** opens the existing type picker, not a new insertion system.
