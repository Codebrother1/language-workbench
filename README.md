...
`OPENAI_MODEL` supplies the centralized environment default (otherwise `gpt-4.1-mini`); no key defaults to Offline conservative. Application routing preferences can override that default. The environment default remains visible even when a previously cached model catalog omits it. An open browser refreshes server-owned provider status on focus, while visible, and before each Lab request; a temporary catalog failure displays unavailable rather than falsely claiming Offline mode. These status checks do not invoke a paid model request. Keys are environment-only, never written by the app to SQLite or entered through the browser. Provider settings expose configuration status and, for keys longer than four characters, only the final four characters—not the raw key.
...
- **New thought** accepts native typing or dictation in a textarea. Choose Draft or Parked; Enter adds a Freeform section there and leaves the capture field ready; Shift+Enter adds a line. The first draft thought fills an untouched empty starter section. Classification can come later. **Add draft section** and Add before/after open the role picker; **Add parked thought** captures prose directly in the parked area.
- **Park thought** moves a section into the visible Parked Thoughts area below the draft without changing its ID, prose, notes, role, workbench, variants, or model choice. A parked thought stays editable in Workbench and is excluded from Document View, reader word count, text/Markdown export, and copy-all. Parked thoughts can remain Ungrouped or belong to one named, collapsible group; groups and their order persist, and cards reorder within their group. **Include in draft** requires an explicit position, identified by label or opening prose, before a draft section or at draft end. Re-parking returns a grouped thought to its prior group.
- **Layout** offers Writing, Review, Workbench only, All panes, and Reset layout. Drag a divider, or focus it and use Left/Right arrow keys, to resize visible panes. The Show/Hide buttons recover any pane; at least one remains visible. Widths and visibility persist. **Focus preview** temporarily gives Preview the full workspace; **Restore panes** brings back the exact saved arrangement. Workbench's **Draft** and **Parked** count buttons jump within its own scroll area without changing section placement.
- Comfortable puts reader-facing Writing ahead of the separately labeled private **Storyboard note** and secondary role. Name or rename a card inline from its header; labels are optional and never change its prose or semantic role. Overview prioritizes the custom short label (or a useful opening excerpt) alongside the section number and secondary role. Cards with saved variants, Structure work or Lab runs show small links back to that existing work. Dragging shows the source and insertion marker before committing a reorder; Move Up/Down remain in section options. **Add before / Add after** opens the existing type picker. The editor toolbar supports heading, bullets and numbered lists; typing `- ` or `1. ` at a paragraph start also starts a list. List structure stays in the section through reload/placement changes and exports as readable text or Markdown.
