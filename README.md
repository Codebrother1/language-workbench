...
This repository is a working implementation, not a claim that every deep rhetorical workflow or dedicated control surface is complete. The current **Timeline / Relational Correction** milestone makes Workbench the default view: ordered section cards, first-class Storyboard notes, one editable prose preview, and section-local assistance. Segue comparison reads adjacent sections without gaining permission to edit them. Word/Phrase Lens, the Personal Writing Library, scoped guides, and Structure remain available in the same application—not parallel editors or a new canonical store. See the [relational editing handoff](docs/RELATIONAL-EDITING.md), [workbench guide](docs/WORKBENCHES.md), [personal-library and Structure guide](docs/PERSONAL-LIBRARY.md), and [Known limitations](#known-limitations). Current verification: **343 unit/integration tests, 71 browser tests, TypeScript, production build and compiled-server restart/security smoke passed**. See [RELATIONAL-VERIFICATION.md](docs/RELATIONAL-VERIFICATION.md); older records remain historical evidence.
...
- **Workbench** is the primary default, including for older settings without a saved view. **Document** is the alternate view of the same editor. View, Comfortable/Overview density, preview visibility, and Inspector visibility persist as global layout preferences. Hide preview to give the cards more room; Edit full prose reveals and focuses the existing editor.
- Comfortable cards show up to seven lines of prose. The selected card exposes its editable **Storyboard note**; other Comfortable notes preview four lines. Overview previews two lines. Cards show semantic role and an explicit section-model override badge when set. **Add before / Add after** opens the existing type picker, not a new insertion system.
...
**Current Timeline / Relational Correction verification passed: 343 unit/integration and 71 browser tests, TypeScript, production build and restart/security smoke.** Details are in [docs/RELATIONAL-VERIFICATION.md](docs/RELATIONAL-VERIFICATION.md). The earlier first-use/wayfinding checkpoint recorded **291 unit/integration tests, 58 browser tests, TypeScript, the production build, and the real-process restart/security smoke check**; see [docs/WAYFINDING.md](docs/WAYFINDING.md). That record and the prior 267/45 unrestricted-composition record remain unchanged and do not verify the new pass.
...
- **Current Timeline / Relational Correction:** [RELATIONAL-EDITING.md](docs/RELATIONAL-EDITING.md) is the product/engineering handoff; [RELATIONAL-VERIFICATION.md](docs/RELATIONAL-VERIFICATION.md) is **pending finalization** by the verification owner.
