...
import { z } from 'zod';
import { aiResponseSchema, radarItemSchema, uid, validateAIRequest, type AIRequest, type AIResponse, type LanguageRadarItem, type LLMProvider } from '@workbench/domain';
import { developerInstructions, forbiddenPhrases, validateProviderResponse } from './provider-policy.js';
...
const cultureSchema = z.object({ items: z.array(radarItemSchema.extend({ sources: z.array(z.object({ title: z.string(), url: z.string() })) })).max(10) });
export type OpenAIProviderOptions = { apiKey: string; model?: string; client?: OpenAI };
...
export class OpenAIProvider implements LLMProvider {
  ...
  constructor({ apiKey, model = 'gpt-4.1-mini', client }: OpenAIProviderOptions) {
    this.client = client ?? new OpenAI({ apiKey, timeout: 60_000, maxRetries: 1 });
    this.model = model;
  }
  async run(request: AIRequest): Promise<AIResponse> {
    ...
    const context = { ...request.readContext,
      knowledgePacks: request.readContext.knowledgePacks.filter(p => p.enabled),
      approvedLanguage: request.readContext.approvedLanguage.filter(r => r.status === 'saved'),
    };
    const response = await this.client.responses.parse({
      ...
      input: [
        ...
        { role: 'user', content: JSON.stringify({
          ...
          action: request.action, stage: request.stage, instruction: request.instruction,
          humanAnswer: request.answer, controls: request.controls, variantCount: request.variantCount,
          forbiddenPhrases: forbiddenPhrases(request),
        }) },
      ],
      text: { format: zodTextFormat(aiResponseSchema, 'writing_response') },
    });
    if (!response.output_parsed) throw new Error('No valid structured response');
  ...
  async researchCulture(query: string): Promise<LanguageRadarItem[]> {
      ...
      for (const part of output.content) {
        ...
        for (const annotation of part.annotations) {
          if (annotation.type === 'url_citation') {
            ...
            if (['http:', 'https:'].includes(url.protocol)) citations.set(annotation.url, { title: annotation.title || url.hostname, url: annotation.url });
          }
        }
      }
    }
    if (!citations.size || !research.output_text.trim()) throw new Error('Research returned no verifiable web citations');
    const extracted = await this.client.responses.parse({
      ...
      input: [
        ...
        { role: 'user', content: JSON.stringify({ query, report: research.output_text, citations: [...citations.values()] }) },
      ],
      text: { format: zodTextFormat(cultureSchema, 'culture_research') },
    });
    const { items } = cultureSchema.parse(extracted.output_parsed);
    return items.map(item => {
      if (!item.sources.length || item.sources.some(source => !citations.has(source.url))) throw new Error('Uncited culture result');
      return radarItemSchema.parse({ ...item, id: uid(), status: 'maybe', discoveredAt: now, lastVerifiedAt: now,
