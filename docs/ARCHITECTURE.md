...
This describes the implemented local application and its boundaries, not a roadmap presented as finished functionality. The current refinement builds on the tagged Timeline / Relational Correction checkpoint `b5e7279`. Installation and operational commands are in [README.md](../README.md). The prior product/engineering handoff is [RELATIONAL-EDITING.md](RELATIONAL-EDITING.md); current checks are in [REFINEMENT-VERIFICATION.md](REFINEMENT-VERIFICATION.md); earlier workflows remain in [WORKBENCHES.md](WORKBENCHES.md) and [PERSONAL-LIBRARY.md](PERSONAL-LIBRARY.md). One editor and the existing workbench ownership model are retained. The prior checkpoint verification remains in [RELATIONAL-VERIFICATION.md](RELATIONAL-VERIFICATION.md): 343 unit/integration and 71 browser tests passed, with type checks, build and restart smoke.
...
- **Settings:** Style DNA, packs, radar, theme, optional application/type/task routing defaults, and optional `layout` (`primaryView`, `density`, three pane visibility flags, and pane width weights). Missing layout defaults to Workbench / Comfortable with all three panes visible at 50/30/20 weights. Global settings are not embedded in document export; full database backup includes them.
...
`section-boundary.ts` enforces the P0 invariant: an ordinary local document-changing transaction must retain the exact ordered section-ID list. Invalid identities are rejected even on authorized/history paths. Native cross-section editing is refused with guidance; selection, navigation and copy are allowed. Handlers cover text input, paste, Backspace/Delete, `beforeinput`, and cut; the transaction filter is the backstop. A boundary-wrapped selection overlapping only one section's editable content is clamped to that content. A neighboring wrapper edge with no selected prose does not make it a cross-section edit; the transaction backstop also checks that all neighboring section nodes remain unchanged. Rich clipboard `writingSection` wrappers are unwrapped into the existing section instead of minting sections. Actual cross-section prose selections are never reinterpreted as local targets.
...
`apps/web/src/section-purpose.ts` is the single teaching catalog for the 27 existing section kinds. Each entry has a name, short description, rhetorical job, usage cue, and category. The insertion picker, Semantic kind selector, scoped style-guide selector, section-type model selector, and command search read from it. Hover and keyboard focus reveal the selected concept's help without changing the native selectors, section data, editor focus policy, or insertion behavior. This metadata can also support later contextual help or onboarding; it does not prescribe the writer's structure.

`App.tsx` retains a single `EditorContent` and TipTap `EditorView`. Clicking the selected Comfortable card's Writing area reparents that editor DOM into the card; Edit in preview and Document View return it to its original mount. This preserves editor state, native clipboard behavior, history, and the section boundary guard. `useWorkspace` still writes editor updates into `Document.sections`; card excerpts and the plain-text assembled readout are projections. No second editor or prose store is created. The initial untouched Freeform section receives the first New thought; subsequent captures use the existing section insertion operation. Role changes, notes, and reorders retain section IDs. `Settings.layout` persists through the existing settings queue. Overview uses compact prose and note cues; the selected card's options close when density changes. The timeline takes more width than preview on wide screens. Model badges show explicit section overrides, not a second resolver. Add before/after routes into the existing insertion picker.

The dock shell keeps all three pane components mounted while CSS hides a pane. `DockDivider` changes only width weights in `Settings.layout`, with pointer capture and keyboard arrows; it never touches `Document.sections`. Presets update layout settings without cloning editor or Inspector state. At medium widths the Inspector stacks beneath Workbench and Preview; at narrow widths all visible panes stack without a fixed Workbench overlay. The Workbench Draft/Parked buttons scroll its own pane to the corresponding area and do not alter placement. Clicking Document View while Preview was hidden reveals that pane for the current visit; returning to Workbench restores the saved hidden state. Pane reordering remains deferred.
